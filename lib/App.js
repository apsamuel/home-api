...
app.get('/api/resume/workhistory/stats', async (req, res) => {
  ...
  // const results = {}
  // for (const workHistoryItem of workHistory) {
  //   const { companyName, companyId, companyType, companyStartDate, companyEndDate } = workHistoryItem
  //   results[companyId] = {
  //   }
  //   const startDate = new Date(companyStartDate).getTime()

  //   const endDate = companyEndDate === 'current'
  //     ? new Date().getTime()
  //     : new Date(companyEndDate).getTime()
  //   // console.log(`start ${startDate} end ${endDate}`);
  //   const diffTime = endDate - startDate
  //   results[companyId]['companyName'] = companyName
  //   results[companyId]['companyTimeServed'] = diffTime / oneDay
  //   results[companyId]['companyTechStack'] = []
  //   results[companyId]['companyTechTypes'] = [];

  //   results[companyId]['companyTotalResponsibilities'] = 0

  //   // get the role history for company
  //   const roleHistory = await queryItems('roleHistory', 'roleHistoryIndex', {
  //     companyId
  //   })
  //   console.log(
  //     `there are ${roleHistory.Items.length} roles associated with this position`
  //   );
  //   results[companyId]['roleStats'] = []
  //   const roleElapsedTimes = []
  //   for (const roleHistoryItem of roleHistory.Items) {
  //     const { roleName, roleId, roleStartDate, roleEndDate, roleTech, roleResponsibilities } = roleHistoryItem
  ...
  //       roleId,
  //       roleName,
  //       roleTimeServed: diffTime / oneDay
  //     }
  //     roleElapsedTimes.push(diffTime / oneDay)
  //     results[companyId]['roleStats'].push(roleStats)
  //     results[companyId]['companyTotalResponsibilities'] += roleResponsibilities.length

  //     // generate data for pie graphs, etc.
  //     const test = await statisticsForRoleTech(roleTech)
  ...
  //       if (! results[companyId]['companyTechStack'].includes(name)) {
  //         results[companyId]['companyTechStack'].push(name)
  //       }

  //       if (! results[companyId]['companyTechTypes'].includes(tech.roleTechType)) {
  //         results[companyId]['companyTechTypes'].push(tech.roleTechType)
  //       }
  //     }
  //   }

  //   results[companyId]['companyRoleAverageTimeServed'] = average(roleElapsedTimes)
